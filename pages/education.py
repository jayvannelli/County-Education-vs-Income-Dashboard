...
def main():
    ...
    st.subheader(
        f"{state_selection} Associate Degree by County ({'%' if display_type == 'Percentage' else 'Quantity'})"
    )
    ...
    st.subheader(
        f"{state_selection} Bachelor Degree by County ({'%' if display_type == 'Percentage' else 'Quantity'})"
    )
